...
(define (funCaseHelper cs)
  (match cs
    [(cons (FunPlain xs e) plainlst)
     (let ((fplain (gensym)))
       (seq (Mov rax (length xs))
            (Cmp r12 rax)
            (Je fplain)
            (funCaseHelper plainlst)
            (Ret)
...
(define (compile-apply f es e c)
  ...
  (let ((returnaddress (gensym))
        (loopbegin (gensym))
        (loopend (gensym)))
    (seq  (Lea rax returnaddress)
          (Push rax) 
          (Mov r12 (length es))
